...
```text
  ...
  encrypt_fields.js          (encrypts with your PUBLIC key)
      v
  publish_motherduck.js      (or publish_gsheets.js)
      v
...
round. That is what makes it a live delivery dashboard and not just a dump.
Every terminal path routes through it, including timeouts and delivery
failures, so a break-off still produces a row.
...
private key can read them back. `publish_gsheets.js` is still here for projects
that need a Google Sheet instead.

## The worked example

`flows/` ships the same instrument in two languages, both generated by
`just build-demo-flow`:

| File | |
| --- | --- |
| `data_use_demo_en.json` | English - Research Staff Training 2026, Jaipur |
| `data_use_demo_es.json` | Spanish - Foro Nacional de Datos, Bogota |
| `foro_nacional_datos_source.json` | The as-ran Bogota original, kept unmodified as provenance |

It is a methods demonstration: respondents are randomised into two arms that get
the **same four questions** in different formats, and the closing message reveals
the experiment.
...
graph lands in both and neither can quietly drift from the other. Read
`scripts/build_data_use_demo.py` first if you are building your own.
